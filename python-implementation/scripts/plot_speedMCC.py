    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            ...
            if linesplit[0][0:4] == "mout":
                ...
                filename = "../results/dataparallel/dataparallel-abp-{}-{}-{}-{}-{}-{}-{}-{}nodes-{}cores.txt".format(linesplit[0], linesplit[1], linesplit[2],
...
if __name__ == "__main__":
    ...
    baseline, lsh, slsh, slsh_base = read_mcc_tuning_file("../results/24cores-2nodes-dataparallel-tuning-3030.txt", p, n, nodes, cores)
    ...
    plt.errorbar(slsh_base[0][1], slsh_base[1], xerr=[[slsh_base[0][0]], [slsh_base[0][2]]], fmt='o', c='green',
                 capsize=4, ms=10, mew=4, capthick=2, elinewidth=2, label="SLSH onset")
    plt.errorbar([e[0][1] for e in lsh], [e[1] for e in lsh], xerr=[[e[0][0] for e in lsh], [e[0][2] for e in lsh]],
                 c='green', fmt='x', capsize=4, ms=10, mew=4, capthick=2, elinewidth=2, label="LSH")
    plt.errorbar([e[0][1] for e in slsh], [e[1] for e in slsh], xerr=[[e[0][0] for e in slsh], [e[0][2] for e in slsh]],
                 c='red', fmt='|', capsize=4, ms=10, mew=4, capthick=2, elinewidth=2, label="SLSH")
