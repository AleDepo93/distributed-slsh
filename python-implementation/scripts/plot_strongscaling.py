...
def plot_strongscaling():
    ...
    for index in range(len(n_list)):
        ...
        # Plot DSLSH.
        custom_plot(index, np.array(worker_list)*8, [x[1] for x in array], [[x[0] for x in array], [x[2] for x in array]], r'Number of overall processors $\nu p$', "Max. Comparisons",
                    r'Strong scaling $n=$'+"{}".format(n), True, labelname="DSLSH", dotted=True)

        # Plot baseline.
        baseline = n / np.array(worker_list) / 8
        custom_plot(index, np.array(worker_list)*8, baseline, baseline, r'Number of overall processors $\nu p$', "Max. Comparisons",
