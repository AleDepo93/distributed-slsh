...
def execute_middleware(node_addresses, cores, n, d, k, X=[], labels=[], queries=[], filename="", synchronous=False, prediction=False):
    """
    ...
    if prediction==True and (X==[] or labels==[]):
        raise ValueError("Prediction from filename not supported. Labels and X must be provided in advance.")
    ...
    """ Processes setup. """
    # Spawn forwarder.
    forwarder_queue = multiprocessing.Queue()  # Use a queue to send the queries to the forwarder.
    forwarder = multiprocessing.Process(target=execute_forwarder, args=(
        node_addresses, forwarder_queue))
    forwarder.start()
    ...
    reducer_queue = multiprocessing.Queue()  # Use a queue to retrieve the queries' outputs from the reducer.
    reducer = multiprocessing.Process(target=execute_reducer, args=(
        node_addresses, k, reducer_queue, synchronous, prediction))
    reducer.start()
    ...
    node_addresses = [(node_address[0], node_address[1] + 2) for node_address in node_addresses]
    ...
    if filename != "":
        queries = send_by_contiguous_slices(n, d, nodes, X=X, labels=labels, filename=filename)
    else:
        send_by_contiguous_slices(n, d, nodes, X=X, labels=labels, filename=filename)
    ...
    # TODO: in a real application the queries would come from a client.
    ...
    if synchronous:
        for i in range(len(queries)):
            ...
            if counter % sampling_ratio == 0:
                print("Start to synchronously process query {}.".format(counter))
            ...
            queries[i] = query
            query.time_middleware_end = time()  # This query's processing has ended.
    ...
    # Asynchronous query execution.
        ...
        for query in queries:
            ...
            query.time_middleware_start = time()  # Start measuring time for this query.
            forwarder_queue.put(query, block=True)
        ...
        while True:
            ...
            queries[counter] = query  # Retrieve the query and substitute it in the list.
...
def synchronize_table_construction(nodes):
    ...
    for node in nodes:
        ...
        if not isinstance(message, str):
            raise ValueError("Wrong table construction synchronization message received.")
        else:
