...
def send_data(data, send_socket):
    ...
    size = len(pickled)
    send_socket.sendall(bytearray(struct.pack("i", size))) # Send size separately first. This takes 4 bytes.
...
def create_socket_connections(node_addresses):
    ...
    for node_address in node_addresses:
        node = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Use IPv4, TCP.
        node.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
