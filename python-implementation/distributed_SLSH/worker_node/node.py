...
'''
    ...
    NOTE: MPI for Python is experimental (you easily get NotImplementedError exceptions) and
    not so well documented. Hence, processes are used here (threads have GIL issues).
'''
...
def share_dataset(X):
    ...
    dataset = multiprocessing.Array('d', np.reshape(np.transpose(X), size), lock=False)

    return dataset, np.shape(X)[0], np.shape(X)[1]


def spawn_cores(cores, barrier, seeds, m_out, L_out, m_in, L_in, dataset, dataset_shape, H_out, H_in, alpha, k, dataparallel):
    '''
    ...
    :param dataparallel: flag for whether to parallelize on data rather than on tables.

    :return: (list of processes, list of pipes)
    '''

    tableparallel = not dataparallel
    ...
    for i in range(0, cores - 1):
        ...
        if tableparallel:
            points_slice = range(dataset_shape[1])  # Use the entire dataset on each core.
        elif dataparallel:
            points_slice = range(i * ceil(dataset_shape[1] / cores), ceil(dataset_shape[1] / cores) * (i + 1)) # Split the dataset.

        # Each process calls execute_core, which build the tables and then waits for queries.
        process = multiprocessing.Process(target=execute_core, args=(
            core_pipe, barrier, seeds[i+1], m_out, n_tables, m_in, L_in, dataset, dataset_shape, H_out, H_in, alpha, k, points_slice))
...
def execute_queries(processes, pipes, T_out, dataset, dataset_shape, g_out, T_in, g_in, k, queries=[], distributed=False, interface=None, labels=[], exhaustive=False):
    '''
    ...
    if not distributed:
        ...
        queries = []  # Store the queries and return them for instrumentation purposes.

    if exhaustive:
        # Set up scikit model.
        scikitmodel = NearestNeighbors(n_neighbors=k, algorithm='brute', metric='manhattan', n_jobs=processes)
    ...
    while True:
        ...
        query_measurement_list = []  # Store the processing times across all cores.
        ...
        if slsh:
            ...
            root_output = slsh_querying(query, T_out, g_out, T_in, g_in, k, selector)
            ...
            while retrieved_results < other_cores:
                active_pipes = multiprocessing.connection.wait(pipes, timeout=-1)  # Return immediately
                for pipe in active_pipes:
                    core_output = pipe.recv()  # Block if the core has not finished yet.
                    query_results.append(core_output[0])  # Append the query result.
                    query_measurement_list.append(core_output[1])  # Append the query measurements.
                    retrieved_results += 1
            ...
            local_query_result = np.array(selector.filter(L1, k, query), dtype=int)

        else:
            # Exhaustive search.
            #TODO: since scikit's parallelism is highly inefficient, implement data parallelism on it (split dataset to cores).
            local_query_result = scikitmodel.kneighbors(X=query.point.reshape(1, -1), return_distance=False)[0].astype(int)

        # Stop measuring the intranode total query processing time.
        time_node_end = time()

        # Keep as query the one with the max query time across all cores.
        query = max(query_measurement_list, key=lambda q: q.time_slsh_end - q.time_slsh_start)
        query.comparisons = max([q.comparisons for q in query_measurement_list])  # Store max number of comparisons per query.
        query.avgcomparisons = np.mean([q.comparisons for q in query_measurement_list])
        query.mincomparisons = min([q.comparisons for q in query_measurement_list])
        query.time_node_start = time_node_start
        query.time_node_end = time_node_end

        # Update query output.
        query.neighbors = indices_to_points(local_query_result, dataset, dataset_shape, exhaustive=exhaustive)
        ...
        if not distributed:
            ...
            # INFO.
            if query_number % sampling_ratio == 0:
                print("Query input: {} \nQuery output: {}".format(query.point, indices_to_points(local_query_result, dataset, dataset_shape, exhaustive=exhaustive)))
        ...
        # Send the output of the current query to the middleware in the distributed setting.
            ...
            # INFO.
            if query_number % sampling_ratio == 0:
                print("Query input: {} \nQuery output: {}".format(query.point, query.neighbors))
...
#TODO: make the SLSH parameters named.
def execute_node(cores, k, m_out, L_out, m_in, L_in, H_out, H_in, alpha, X=[], queries=[], distributed=False, port=1025, labels=[], prediction=False, exhaustive=False, dataparallel=False):
    """
    ...
    if distributed and len(labels) > 0:
        raise ValueError("Labels should be provided only for local execution of the system in prediction mode.")
    ...
    if slsh:
        ...
        dataset, n_rows, n_columns = share_dataset(X)  # This is preprocessing time, not included in algorithm measurements.
        ...
        barrier = multiprocessing.Barrier(cores)  # Create a barrier for all the cores.
        ...
        processes, pipes = spawn_cores(cores, barrier, seeds, m_out, L_out, m_in, L_in, dataset, dataset_shape, H_out, H_in, alpha, k, dataparallel)

        # Treat shared dataset as numpy array with no copying.
        dataset_as_ndarray = np.frombuffer(dataset)  # No-copy method to treat dataset as numpy array.

        # The root builds its own tables.

        # Parallelize on tables.
        if tableparallel:
            root_tables = L_out - ceil(L_out / cores)*(cores-1)  # Use the remainder of the table split from the other cores.
            points_slice = range(dataset_shape[1])  # Use the entire dataset.
        # Parallelize by splitting the dataset, as in the distributed system.
        elif dataparallel:
            root_tables = L_out  # Use all the tables on each core.
            points_slice = range(ceil(dataset_shape[1] / cores)*(cores-1), dataset_shape[1] - ceil(dataset_shape[1] / cores)*(cores-1))  # Use the remainder of the dataset split from the other cores

        (T_out, g_out, T_in, g_in) = slsh_indexing(
            m_out, root_tables, m_in, L_in, dataset_as_ndarray, dataset_shape, H_out, H_in, alpha, info=True, outer_points=points_slice)
    ...
    queries = execute_queries(processes, pipes, T_out, dataset_as_ndarray, dataset_shape, g_out, T_in, g_in, k, queries=queries, distributed=distributed, interface=interface, labels=labels, exhaustive=exhaustive)
