...
class TestDistributedlSLSH(unittest.TestCase):
    ...
    def test_distributed_NN(self):
        ...
        temp1, queries = execute_node(cores, k, m_out, L_out, m_in, L_in, H_out, H_in, alpha, distributed=True)
    ...
    def test_distributed_prediction(self):
        ...
        # Execute parallel code.
        temp1, queries = execute_node(cores, k, m_out, L_out, m_in, L_in, H_out, H_in, alpha, distributed=True, prediction=True)

        self.assertTrue(np.shape(queries[0].neighbors[0])[0] == D)
        self.assertTrue(np.array_equal(queries[0].neighbors_labels[0], 1))
