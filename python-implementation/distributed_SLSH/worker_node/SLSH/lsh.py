# Code adapted from the SLSH_pyhthon_akshat repository.
...
def L1(x, y):
    ...
    # Linalg's norm is a factor 2 faster than this (this is the bottleneck in query resolution).
    # np.sum([abs(a - b) for (a, b) in zip(x, y)])
...
def lsh_indexing(m, L, X, X_shape, points, H, info=False):
    ...
    g = [None] * L # Store the g instance for each table. g is the concatenation of m hashes from H.
    T = [None] * L
    ...
    for l in range(L):
        ...
        for i in points:
            x = X[i*X_shape[0]:(i+1)*X_shape[0]] # Access the current datapoint.
...
def lsh_querying(q, T, g, k, selector, d=L1):
    ...
    for l in range(L):
        ...
        if key in T[l]:
            selector.add(T[l][key]) # Add to the selector the lists of candidates.
