# Heavily adapted from the SLSH_pyhthon_akshat repository.
...
class NearestPoints(Selector):
    ...
    def add(self, items):
        ...
        self.data.append(items) # Add whole lists to move the linear time to the filter function.
    ...
    def filter(self, d, k, q, save_comparisons=False):
        ...
        if not self.prediction:
            ...
            try:
                ...
                # If there are no points for the current query, return nothing.
                return [np.array([])]
    ...
    def filter_labeled(self, d, k, q):
        ...
        complete_list = iterate_and_remove_duplicates(self.data, prediction=True)
    ...
    def filter_faster(self, complete_list, d, k, q):
        ...
        #TODO: this is also way slower then the kneighbor search.
        points = np.empty((len(complete_list), self.X_shape[0]))
        counter = 0
        for i in complete_list:
            # i is a point index
            points[counter, :] = self.X[i * self.X_shape[0]:(i + 1) * self.X_shape[0]]
            counter += 1

        scikitmodel = NearestNeighbors(n_neighbors=k, algorithm='brute', metric='manhattan')
        scikitmodel.fit(points)
        knn_results = scikitmodel.kneighbors(X=q.reshape(1, -1), return_distance=False)[0].astype(int)
...
def iterate_and_remove_duplicates(data, prediction=False):
    ...
    #TODO: this is the bottleneck (factor 10) of linear search on large queries.
