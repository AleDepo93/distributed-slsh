...
def slsh_indexing(m_out, L_out, m_in, L_in, X, X_shape, H_out, H_in, alpha, info=False, outer_points=[]):
    """
    ...
    g_out, T_out = lsh_indexing(m_out, L_out, X, X_shape, outer_points, H_out, info=info)
    T_in = [None] * L_out  #inner hash tables
    g_in = [None] * L_out  #inner hash functions

    if info:
        print("A core built the outer tables and is now starting inner tables.")
    ...
    for l in range(L_out):
        ...
        for key in T_out[l]:
            ...
            if len(T_out[l][key]) >= (alpha * X_shape[1]):
                ...
                g_in[l][key], T_in[l][key] = lsh_indexing(m_in, L_in, X, X_shape, inner_points, H_in)
