...
class MiddlewareInterface():
    ...
    def __init__(self, address=('', 1025)):
        ...
        forwarder_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Use IPv4, TCP.
        forwarder_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # Avoid "address already in use" issues.
        forwarder_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1) # Avoid problem described in [1]
        forwarder_socket.setblocking(True)  # Use blocking operations.
        forwarder_socket.bind(forwarder_address)  # Receive on all available interfaces, on the specified port.
        forwarder_socket.listen(1)

        # Initialize receiver socket.
        reducer_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Use IPv4, TCP.
        reducer_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Avoid "address already in use" issues.
        reducer_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
        reducer_socket.setblocking(True)  # Use blocking operations.
        reducer_socket.bind(reducer_address)  # Receive on all available interfaces, on the port for the forwarder+1.
        reducer_socket.listen(1)

        # Initialize root socket.
        root_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Use IPv4, TCP.
        root_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Avoid "address already in use" issues.
        root_socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
        root_socket.setblocking(True)  # Use blocking operations.
        root_socket.bind(root_address)  # Receive on all available interfaces, on the port for the forwarder+2.
        root_socket.listen(1)

        # Accept connections.
        root_connection, address = root_socket.accept()
        root_connection.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
        reducer_connection, address = reducer_socket.accept()
        reducer_connection.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
        forwarder_connection, address = forwarder_socket.accept()
        forwarder_connection.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # Avoid problem described in [1]
