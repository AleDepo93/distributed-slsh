...
%   Unify channel nomenclature and ordering, then downsample to 100Hz to facilitate merging. Input is a struct in the same format of edf_to_stuct.m.
%   epoch_length is the length of an epoch in seconds. Epochs will be later used for artifact detection.
    ...
    % Downsample to 100 Hz, preliminaries.
    ...
    for k = 1:num_chans
        ...
        if(~isnan(s_to_m(k)))
            output.matrix(s_to_m(k), :) = resample(input.matrix(k, :),TARGET_FREQUENCY,FREQUENCY);  % Downsample to 100 Hz.
